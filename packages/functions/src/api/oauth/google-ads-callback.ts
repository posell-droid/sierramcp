 ...
 *
 * Lambda handler for receiving Google's OAuth redirect after user authorization.
 * Validates state, exchanges authorization code for tokens via Google OAuth 2.0, and stores credentials.
 *
 * Key differences from Amazon:
 * - Uses standard OAuth 2.0 code flow with Google's token endpoint
...
/**
 * Exchange authorization code for access and refresh tokens via Google OAuth 2.0
 */
...
export async function handler(
  ...
  // Get Google Ads credentials from SST Resource
  ...
  try {
      ...
      if (oauthState.environment.secretArn) {
        ...
        const result = await secretsClient.send(
          new CreateSecretCommand({
            Name: secretName,
            SecretString: secretValue,
            Description: `Google Ads credentials for ${oauthState.environment.application.name} - ${oauthState.environment.environment}`,
            Tags: [
    ...
    console.log("Google Ads OAuth flow completed successfully");
