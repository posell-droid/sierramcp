...
function useToolsList(applicationId: string) {
  const [data] = useState<Tool[]>([
    {
      id: "1",
      name: "Search Knowledge Base",
      httpMethod: "GET",
      endpointPath: "/api/v1/knowledge/search",
      status: "Published",
      safety: "Read-only",
    ...
    },
    {
      ...
      httpMethod: "PATCH",
      endpointPath: "/api/v1/tickets/{id}/status",
      status: "Published",
      safety: "Write",
    ...
    {
      id: "5",
      name: "List Recent Tickets",
      httpMethod: "GET",
      endpointPath: "/api/v1/tickets/recent",
      status: "Draft",
      safety: "Read-only",
    ...
    },
    {
      ...
      name: "Delete Ticket",
      httpMethod: "DELETE",
      endpointPath: "/api/v1/tickets/{id}",
      status: "Draft",
      safety: "Write",
...
export default function ToolsListPage({ params }: { params: { id: string } }) {
  ...
  const { data: tools, isLoading } = useToolsList(params.id)
  const [searchTerm, setSearchTerm] = useState("")

  const handleRowClick = (toolId: string) => {
    router.push(`/applications/${params.id}/tools/${toolId}/edit`)
  }

  const handleAction = (action: string, tool: Tool, event: React.MouseEvent) => {
    event.stopPropagation()

    switch (action) {
      case "edit":
        router.push(`/applications/${params.id}/tools/${tool.id}/edit`)
        break
      case "test":
      ...
      <div className="min-h-screen bg-muted/30 p-6">
        <div className="mx-auto max-w-7xl">
          <Suspense fallback={<div className="h-6 mb-6" />}>
            <div className="mb-6">
              <ToolsBreadcrumb applicationId={params.id} applicationName={application?.name} />
            </div>
          </Suspense>
