...
function BreadcrumbNav({ id }: { id: string }) {
  return (
    <Breadcrumb className="mb-6">
      <BreadcrumbList>
        ...
        <BreadcrumbItem>
          <BreadcrumbLink href={`/mcps/${id}`}>Finance Analytics</BreadcrumbLink>
        </BreadcrumbItem>
        <BreadcrumbSeparator>
          <ChevronRight className="h-4 w-4" />
        </BreadcrumbSeparator>
...
export default function MCPToolsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
